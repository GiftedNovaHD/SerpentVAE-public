  ...
  for sample in batch: 
    try:
      ...
      with torch.no_grad():
        ...
        # Stack all transformed frames to create a batch tensor of shape 
        ...
        if isinstance(batch_features_output, Tensor):
          sequence_features = model.forward_head(batch_features_output, pre_logits=True)
        else: 
          sequence_features = torch.stack([model.forward_head(feat, pre_logits=True) for feat in batch_features_output], dim=0)
        
        batch_features.append(sequence_features.cpu()) # Move to CPU to free GPU memory
