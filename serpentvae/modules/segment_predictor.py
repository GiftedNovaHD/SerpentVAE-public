...
class DecoderSegmentPredictor(nn.Module):
  ...
  def forward(self,
    ...
    hidden_states = self.hidden_mlp(hidden_states)
    hidden_states = self.hidden_up_proj(hidden_states)

    segment_pred = einx.dot("batch seq_len [concept_dim], batch seq_len [concept_dim] -> batch seq_len 1", concept_tokens, hidden_states)
