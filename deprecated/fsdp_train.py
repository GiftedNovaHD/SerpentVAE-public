...
def train_fn(model: SerpentVAE,
  ...
  for batch in train_loader: 
    ...
    # Use autocast for automatic mixed precision training
    with autocast(device_type="cuda", dtype=getattr(torch, config["dtype"])): 
...
def train(model: nn.Module, optimizer: optim.Optimizer, train_loader: DataLoader, num_epochs: int, config: dict): 
  for epoch in range(1, num_epochs + 1): 
    train_loss = train_fn(model, optimizer, train_loader, epoch, config)
  return 


def eval_fn(model: nn.Module, val_loader: DataLoader, epoch: int, config: dict): 
  model.eval()
  total_loss = 0.0 
...
def eval(model: nn.Module, val_loader: DataLoader, num_epochs: int, config: dict): 
  for epoch in range(1, num_epochs + 1): 
    _ = eval_fn(model, val_loader, epoch, config)
  return 

def save_checkpoint(model: nn.Module, epoch: int): 
  ...
  if int(os.environ.get("RANK", 0)) == 0: 
    filename = f"checkpoint_epoch_{epoch}_{get_date_and_run()}.pt"
    torch.save(state, filename)
    print(f"Checkpoint saved: {filename}")
def training_loop(model: nn.Module, optimizer: optim.Optimizer, train_loader: DataLoader, val_loader: DataLoader, num_epochs: int, eval_freq: int, config: dict): 
  for epoch in range(1, num_epochs + 1): 
    train_loss = train_fn(model, optimizer, train_loader, epoch, config) 
    if epoch % eval_freq == 0: 
      val_loss = eval_fn(model, val_loader, epoch, config)
